...
export const meetingsRouter = createTRPCRouter({
  generateChatToken: protectedProcedure.mutation(async ({ ctx }) => {
    // Create user with proper permissions for channel creation
    await streamChat.upsertUser({
      ...
      role: "user", // Use 'user' role for proper permissions
    });

    // Generate token (simple version without custom claims)
  ...
  getTranscript: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (!existingMeeting.transcribleUrl) {
        return [];
      }

      const transcript = await fetch(existingMeeting.transcribleUrl)
        .then((res) => res.text())
